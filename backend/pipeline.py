...
Replaces the old phase-based pipeline with a multi-round character loop:
  1. Narrator resolves player intention → segments
  2. Character extractor runs for each named character
  3. Per round: activate characters, each generates intention → narrator resolves → extractor updates
  4. Lorebook extractor runs per round
  5. Tick states, combine all segments
"""
