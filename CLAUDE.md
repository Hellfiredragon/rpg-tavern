...
## Rules
- `.claude/rules/backend.md` — Python, FastAPI, data models, DB, tests
- `.claude/rules/frontend.md` — React, Vite, components, frontend state
- `.claude/rules/mcp.md` — MCP tools, world state mutation
- `.claude/rules/pipeline.md` — LLM pipeline, prompts, stages, extractors
- `.claude/rules/tests.md` — Test conventions, helper classes
- `.claude/rules/character_state.md` — Character state system design
...
- ALWAYS create a `docs/plans/current_plan_NNN.md` when agreeing on a new plan with the user. NNN is a zero-padded incrementing number starting at 001. Find the highest existing number with `ls docs/plans/current_plan_*.md` and increment. The plan file must list every step with a status checkbox (`- [ ]` / `- [x]`). Update the checkboxes as steps are completed. Read the active plan at any time with `bash scripts/agent/show_current_plan.sh`.
...
|--------------------------------------------|---------------------------------|
| Python, FastAPI, data models, DB, tests    | `.claude/rules/backend.md`      |
| React, Vite, components, frontend state    | `.claude/rules/frontend.md`     |
| MCP tools, world state mutation            | `.claude/rules/mcp.md`          |
| LLM pipeline, prompts, stages, extractors  | `.claude/rules/pipeline.md`     |
| Writing or modifying tests                  | `.claude/rules/tests.md`        |
...
bash scripts/agent/show_current_plan.sh  # Print the active plan (highest-numbered)
```
...
See `.claude/rules/pipeline.md` for full stage behaviour.
