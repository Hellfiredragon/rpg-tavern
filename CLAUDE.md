...
backend/              Python backend (FastAPI)
  ...
  storage.py          File-based JSON storage (adventures, world state)
  demo.py             Demo data generator (--demo flag)
  static/             Built frontend assets (gitignored, generated by `bun run build`)

frontend/             React + TypeScript (Vite)
  src/
    App.tsx            Router: selectedAdventureId → QuestBoard or AdventureView
...
JSON files in `data/` (configurable via `DATA_DIR` env or `--data-dir` flag). Structure: `data/adventures/<id>/adventure.json`. Tests use `data-tests/` which is wiped before each test run.

Adventures have a `variant` field: `"template"` (default, editing only) or `"running"` (active play). Running adventures created via "Embark" carry a `template_id` back-reference to their source template.
