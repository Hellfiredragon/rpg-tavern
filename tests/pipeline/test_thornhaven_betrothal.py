...
async def test_thornhaven_betrothal(storage: Storage) -> None:
    ...
    s.npc_intent(
        "Lord Cael",
        "I'll rise to greet her properly — whatever comes next can wait.",
    )
    s.narrate(
    ...
    s.npc_says("Lord Cael", "Lady Isolde. You are welcome at Thornhaven.", mood="formal")
    s.narrate("He pours wine into the cup beside her place without being asked, then sits.")
    s.npc_extractor("Lord Cael", amplify=["duty to lineage"])
    ...
    s.npc_extractor(
        "Brenna",
        suppress=["excitement about feast"],
        amplify=["protectiveness of Isolde"],
    )
    s.lore()
    ...
    # places a persona_verbatim beat, no LLM call needed.
    s.persona_says(
    ...
    s.npc_says("Lord Cael", "Lady Lysa. My wife. She died three winters past.", mood="measured")
    ...
    s.npc_extractor("Lord Cael", amplify=["grief over lost wife"])
    # Aldous's overflow at 27 (+3 = 27) does not yet hit 30; resentment stays in
    ...
    s.npc_intent(
        "Brenna",
        "I'll watch Aldous — I know his kind. I'll stay close to my lady.",
    )
    s.narrate(
        "Brenna's expression does not change. Her eyes follow Aldous the way a cat "
        "watches a door."
    )
    # Brenna is silent this round; no npc_says call.  The extractor still runs
    # because she was the activated NPC for this round.
    s.npc_extractor(
        "Brenna",
        amplify=["protectiveness of Isolde", "wariness of Aldous"],
    )
    s.lore()
    ...
    s.npc_says(
        "Lord Cael",
        "Sir Aldous. Your seat awaits you.",
        mood="flat",
    ...
    s.npc_extractor("Brenna", amplify=["protectiveness of Isolde"])
    s.lore()
